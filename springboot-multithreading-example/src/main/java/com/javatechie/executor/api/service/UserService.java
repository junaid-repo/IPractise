...
public class UserService {
	...
	public CompletableFuture<String> saveTeachersDetails(List<ClassDTO> req) {
		long st = System.currentTimeMillis();
		req.stream().parallel().forEach(obj -> {

			// System.out.println("the number of time -> "+i);
			String extRes = comm.callCreateTeacher(obj);

		});
		System.out.println("The total time taken for saveTeachersDetails is--> " + (System.currentTimeMillis() - st));
		return CompletableFuture.completedFuture("completed");
	}
